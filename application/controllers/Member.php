...
class Member extends REST_Controller {
	...
	function send_approved_post()
	{
		...
		if ($validation == 'ok')
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				$content['member_card'] = $query->row()->member_card;
	...
	function send_request_transfer_post()
	{
		$this->benchmark->mark('code_start');
		$validation = 'ok';
		...
		if ($validation == 'ok')
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				$send_email = email_member_request_transfer($content);
			...
			if ($query->num_rows() > 0)
			{
				...
				if ($status == TRUE)
				{
					...
					elseif ($status == 4) // approved
					{
						...
						if ($query->row()->approved_date == '' || $query->row()->approved_date == '0000-00-00 00:00:00')
						{
							$param['approved_date'] = date('Y-m-d H:i:s');
						}
					}
